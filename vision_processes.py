...
def make_fn(model_class, process_name, counter, **kwargs):
    """
    ...
    if kwargs:
        # hack instead of adding **kwargs to every vision model's __init__ method
        model_instance = model_class(gpu_number=gpu_number, **kwargs) 
    else:
...
def clear_all_consumers():
    """
    Clears the consumers. This is useful when the models are updated, and the old models are no longer needed.
    """
    global consumers
    for consumer in list(consumers.keys()):
        del consumers[consumer]
...
def instantiate_consumer(consumer_type, **kwargs):
    """
    Instantiates a consumer of a certain model type, given the model name. This is useful when the consumer should not be instantiated as part of the initial instantiation.
    """
    global consumers

    for model_class_ in list_models:
        if consumer_type in model_class_.list_processes():
            consumers[consumer_type] = make_fn(model_class_, consumer_type, counter=0, **kwargs)
