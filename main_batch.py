...
from utils import seed_everything, log_files
...
def run_program(parameters, queues_in_, input_type_, retrying=False):
    ...
    code_header = f'def execute_command_{sample_id}(' \
                  ...
                  '    # Answer is:'
    # first line can have slightly different signature, replace it dynamically
    if code.split("\n")[0].startswith('def execute_command'):
    ...
    #NOTE what is the purpose of this? What is in the queues?
    ...
    try:
        result = restricted_globals[f'execute_command_{sample_id}'](
        ...
        traceback.print_exc()
        # in case of no multiprocessing, log the error as result
        if not config.multiprocessing:
            return e.__repr__(), code
        # NOTE what is the purpose of this? Why not directly return (None, code)
        if retrying:
            return None, code
    ...
    #NOTE obsolete when executing in restricted environment
...
def main():
    ...
    # queues_in is None in case of config.multiprocessing=False
    ...
    if not config.generate_code:
        precomputed_code_df = pd.read_csv(config.generated_code_file)
    ...
    try:
        with mp.Pool(processes=num_processes, initializer=worker_init, initargs=(queues_results,)) \
            ...
            try:
                ...
                for i, batch in tqdm(enumerate(dataloader), total=n_batches):
                    # adapt namings in batch dict
                    if config.dataset.dataset_name == 'GQA':
                        batch["query"] = batch["question"]
                        batch["query_type"] = batch["question_type"]
                        batch["extra_context"] = [None for _ in range(len(batch))]
                    ...
                    if config.generate_code:
                        ...
                        # access the "code" entry from precomputed_df, where precomputed_df["id"] matches batch["sample_id"]
                        codes = [precomputed_code_df.loc[precomputed_code_df['id'] == sample_id, 'code'].iloc[0] for sample_id in batch['sample_id']]
                    ...
                    if config.execute_code:
                        if not config.multiprocessing:
                            ...
                            results = []
                            for c, sample_id, image, image_name, possible_answers, query in \
                                    zip(codes, batch['sample_id'], batch['image'], batch['image_name'], batch['possible_answers'], batch['query']):
                                result = run_program([c, sample_id, (image, image_name), possible_answers, query], queues_in, input_type)
                    ...
                    if not config.dataset.llm_acc_metric:
        ...
        try:
            accuracy = dataset.accuracy(all_results, all_answers, all_queries, all_possible_answers, all_query_types)
            console.print(f'Final accuracy: {accuracy}')
        except Exception as e:
            print(f'Error computing accuracy: {e}')

    except KeyboardInterrupt:
        print("Interrupt received. Wrapping up and saving results...")
    finally:
        if config.save:
